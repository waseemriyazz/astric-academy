...
class PlanPurchased extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $planNames = $this->payment->plans->pluck('tier_name')->implode(', ');

        return (new MailMessage)
            ->subject('New Plan Added — ' . $this->course->title . ' (' . $appName . ')')
            ->greeting('Hello ' . $notifiable->name . '!')
            ->line('You have successfully added a new plan to a course you are already enrolled in on ' . $appName . '.')
            ->line('---')
            ->line('**Course:** ' . $this->course->title)
            ->line('**Plan(s):** ' . ($planNames ?: 'N/A'))
            ...
            ->line('---')
            ->line('You can access your updated course materials from your student dashboard.')
