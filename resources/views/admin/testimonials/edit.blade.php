...
<div class="max-w-2xl bg-white rounded-2xl shadow-[0_2px_10px_rgb(0,0,0,0.02)] border border-gray-100 p-8">
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" required value="{{ old('name', $testimonial->name) }}" class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm">
            @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>
        
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Role / Designation *</label>
            <input type="text" name="role" required value="{{ old('role', $testimonial->role) }}" class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm">
            @error('role') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>
        
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Testimonial Content *</label>
            <textarea name="content" rows="4" required class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm">{{ old('content', $testimonial->content) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="url" name="image_url" value="{{ old('image_url', $testimonial->image_url) }}" class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm">
            ...
            <div>
                ...
                <input type="number" name="sort_order" min="0" value="{{ old('sort_order', $testimonial->sort_order) }}" class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm">
