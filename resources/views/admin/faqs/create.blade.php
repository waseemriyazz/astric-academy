...
<div class="max-w-2xl bg-white rounded-2xl shadow-[0_2px_10px_rgb(0,0,0,0.02)] border border-gray-100 p-8">
    ...
    <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="question" required value="{{ old('question') }}" class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm" placeholder="e.g. Are the courses self-paced or live?">
        ...
        <div>
            ...
            <textarea name="answer" rows="4" required class="w-full rounded-2xl border-gray-200 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm" placeholder="e.g. We offer both! Most courses are self-paced with weekly live mentorship sessions.">{{ old('answer') }}</textarea>
