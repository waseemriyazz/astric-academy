    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6">
            ...
            <div class="mt-6 pt-6 border-t border-gray-200">
                <button onclick="document.getElementById('edit-profile-section').scrollIntoView({behavior: 'smooth'})" class="inline-flex items-center gap-2 px-4 py-2 bg-brand-600 text-white font-semibold rounded-2xl hover:bg-brand-700 transition shadow-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-200 bg-gray-50">
            <h4 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                <i class="fas fa-book-open text-brand-600"></i>
                My Courses
                <span class="ml-2 px-2.5 py-0.5 bg-brand-100 text-brand-700 text-xs font-bold rounded-full">
                    {{ $courses->count() }}
                </span>
            </h4>
        </div>
            ...
            @else
                <div class="space-y-4">
                    @foreach($courses as $course)
                        <div class="border border-gray-200 rounded-2xl p-5 hover:border-brand-300 hover:shadow-md transition-all duration-200">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1">
                                    ...
                                    <div class="flex flex-wrap items-center gap-3 text-xs">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-brand-50 text-brand-700 font-semibold border border-brand-100">
                                ...
                                <a href="{{ route('student.courses.show', $course->id) }}" class="shrink-0 inline-flex items-center gap-2 px-4 py-2 bg-brand-600 text-white font-semibold rounded-2xl hover:bg-brand-700 transition shadow-sm text-sm">
                                    ...
                                    <div class="space-y-1.5">
                                        @foreach($course->lessons->take(3) as $lesson)
                                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                                <i class="fas fa-circle text-[8px] text-brand-400"></i>
