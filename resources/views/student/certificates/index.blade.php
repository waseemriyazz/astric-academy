                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="mt-3 mb-3">
                            <div class="flex items-center justify-between text-sm mb-1.5">
                                ...
                                <span class="text-brand-600 font-bold">{{ $progressPercent }}%</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-500 {{ $allComplete ? 'bg-green-500' : 'bg-brand-500' }}" style="width: {{ $progressPercent }}%"></div>
                            </div>
                        </div>

                        <!-- Certificate Preview Card (Locked/Unlocked) -->
                        <div class="relative mt-4 rounded-2xl overflow-hidden border {{ $allComplete ? 'border-green-200' : 'border-gray-200' }}">
                            <!-- Mini certificate preview -->
                            <div class="bg-gradient-to-br from-brand-50 via-white to-purple-50 p-5 {{ !$allComplete ? 'opacity-30' : '' }}">
                                <div class="text-center">
                                    <div class="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-brand-600 to-cyan-500 rounded-full flex items-center justify-center {{ !$allComplete ? 'opacity-40' : '' }}">
                            ...
                            @if(!$allComplete)
                                ...
                                <div class="absolute inset-0 flex items-center justify-center z-10">
                                    <div class="text-center">
                                        ...
                                        <p class="text-sm font-bold text-gray-500 bg-white/80 px-4 py-1.5 rounded-2xl shadow-sm inline-block">Complete all lessons to unlock</p>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="shrink-0 flex flex-col gap-2">
                        @if($allComplete && $certificate)
                            <a href="{{ route('student.certificates.download', $course->id) }}" 
                               class="inline-flex items-center justify-center gap-2 px-5 py-3 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold rounded-2xl transition shadow-sm">
                                <i class="fas fa-download"></i> Download PDF
                            </a>
                            <a href="{{ route('student.certificates.show', $course->id) }}" 
                               class="inline-flex items-center justify-center gap-2 px-5 py-3 border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-semibold rounded-2xl transition">
                        ...
                        @elseif($allComplete && !$certificate)
                            <form action="{{ route('student.certificates.generate', $course->id) }}" method="POST">
                                @csrf
                                <button type="submit" 
                                        class="inline-flex items-center justify-center gap-2 px-5 py-3 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-2xl transition shadow-sm w-full">
                                    <i class="fas fa-award"></i> Claim Certificate
                                </button>
                            </form>
                        @else
                            <a href="{{ route('student.courses.show', $course->id) }}" 
                               class="inline-flex items-center justify-center gap-2 px-5 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-2xl transition">
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <a href="{{ route('student.dashboard') }}" 
               class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold rounded-2xl transition shadow-sm">
...
@if(count($certificates) > 0)
    <!-- Verification Info -->
    <div class="bg-gradient-to-br from-blue-50 to-brand-50 border border-blue-200 rounded-2xl p-6">
